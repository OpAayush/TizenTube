...
//
// Persisted to disk so a module/service restart doesn't wipe it. Tizen's
// service host runs a real Node process, so plain fs works; we just need a
// writable directory. __dirname is normally writable for a TizenBrew module
// install, but fall back to the OS temp dir if it isn't.
const STORE_DIR = (() => {
  try {
...
function loadStore() {
  try {
    ...
    // First run, missing file, or corrupt JSON — start fresh.
  }
...
function saveStore() {
  try {
    fs.writeFileSync(
      STORE_PATH,
      JSON.stringify({ config: storedConfig, revision: configRevision }),
    );
  } catch (err) {
