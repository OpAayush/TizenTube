// AST Parser for axotube, used for finding code patterns
...
export function extractAssignedFunctions(code) {
  ...
  estraverse.traverse(ast, {
    enter: function (node) {
      ...
      const rhsSrc = original.slice(
        rhs.range[0] - wrapOffset,
        rhs.range[1] - wrapOffset,
      );
      let inner = null;
      ...
      if (
        ...
        for (let i = 0; i < stm.length; i++) {
          ...
          if (s.type === "ReturnStatement" && s.argument && s.argument.range) {
            inner = original.slice(
              s.argument.range[0] - wrapOffset,
              s.argument.range[1] - wrapOffset,
            );
            break;
          }
        }
      ...
      out.push({
        left:
          node.left && node.left.range
            ? original.slice(
                node.left.range[0] - wrapOffset,
                node.left.range[1] - wrapOffset,
              )
            : null,
        rhs: rhsSrc,
        returned: inner,
