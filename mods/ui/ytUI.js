...
// FILE: mods/ui/ytUI.js

function longPressData(data) {
  // Define the base items list
  const menuItems = [
  ...
  // --- START CHANGE ---
  // Add "Go to Channel" if the endpoint exists
  if (data.channelEndpoint) {
    menuItems.splice(
      1,
      0,
      MenuNavigationItemRenderer("Go to Channel", data.channelEndpoint)
    );
  }
  // --- END CHANGE ---
  ...
  return {
    ...
    showMenuCommand: {
      ...
      menu: {
        menuRenderer: {
          items: menuItems, // Use our modified array
          trackingParams: null,
          accessibility: {
            accessibilityData: {
              label: "Video options",
